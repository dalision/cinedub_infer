...
def sample_k(
        model_fn,  #<class 'stable_audio_tools.models.diffusion.DiTWrapper'>
        noise, 
        init_data=None,
        steps=100, 
        ...
        cond_fn=None,   
        safa_args=None,
        **extra_args # add a. unimodal_model_task=["VA" or "VTA" ]  b.conditioning_inputs c.unimodal_conditioning_inputs
        # dict_keys(['cross_attn_cond', 'cross_attn_mask', 'global_cond', 'additional_cond', 'input_concat_cond', 'prepend_cond', 'prepend_cond_mask', |||| 'cfg_interval', 'scale_phi',  'cfg_scale', 'batch_cfg', 'rescale_cfg'])
    ):
    is_k_diff = sampler_type in ["k-heun", "k-lms", "k-dpmpp-2s-ancestral", "k-dpm-2", "k-dpm-fast", "k-dpm-adaptive", "dpmpp-2m-sde", "dpmpp-2m-sde-long", "dpmpp-3m-sde","dpmpp-2m","dpmpp-2m-sde-multimodal"]
    ...
    if is_k_diff: #True
        ...
        if cond_fn is not None: # is None
            denoiser = make_cond_model_fn(denoiser, cond_fn)

        # Make the list of sigmas. Sigma values are scalars related to the amount of noise each denoising step has
        sigmas = K.sampling.get_sigmas_polyexponential(steps, sigma_min, sigma_max, rho, device=device) # continous from 1000 to 0
        ...
        if sampler_type == "k-heun":
            ...
            return K.sampling.sample_dpmpp_2m(denoiser, x, sigmas, disable=True, callback=callback, extra_args=extra_args)
        elif sampler_type == "dpmpp-2m-sde":
            with torch.cuda.amp.autocast():
                return K.sampling.sample_dpmpp_2m_sde(denoiser, x, sigmas, disable=True, callback=callback, extra_args=extra_args)
        elif sampler_type == "dpmpp-2m-sde-long":
            with torch.cuda.amp.autocast():
                return K.sampling.sample_dpmpp_2m_sde(denoiser, x, sigmas, disable=True, callback=callback, extra_args=extra_args, safa=True,safa_args=safa_args)
        elif sampler_type == "dpmpp-2m-sde-multimodal":
            with torch.cuda.amp.autocast():
                return K.sampling.sample_dpmpp_2m_sde_multimodal(denoiser, x, sigmas, disable=True, callback=callback, extra_args=extra_args)
        elif sampler_type == "dpmpp-3m-sde":
            with torch.cuda.amp.autocast():
                return K.sampling.sample_dpmpp_3m_sde(denoiser, x, sigmas, disable=True, callback=callback, extra_args=extra_args)
    elif is_v_diff:
