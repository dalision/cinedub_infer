# Adapted from Stability AI's stable-audio-tools (MIT) and, in parts, from
# archinet.ai's audio-diffusion-pytorch (MIT). See LICENSES/LICENSE_STABILITY.txt
# and LICENSES/LICENSE_ADP.txt.
import torch
...
from .dit import DiffusionTransformer,DiffusionTransformerCineDub
...
class ConditionedDiffusionModel(nn.Module):
    def __init__(self,
                *args,
                ...
                supports_additional_cond: bool = False,
                **kwargs):
        super().__init__(*args, **kwargs)
        ...
        self.supports_prepend_cond = supports_additional_cond
    ...
    def forward(self,
                ...
                additional_embed: torch.Tensor = None,
...
class ConditionedDiffusionModelWrapper(nn.Module):
    ...
    def __init__(
            ...
            add_cross_attn_cond_ids: tp.List[str] = [],
            global_cond_ids: tp.List[str] = [],
            additional_cond_ids: tp.List[str] = [],
        ...
        self.add_cross_attn_cond_ids = add_cross_attn_cond_ids
        self.global_cond_ids = global_cond_ids
        self.additional_cond_ids = additional_cond_ids
    ...
    def get_conditioning_inputs(self, conditioning_tensors: tp.Dict[str, tp.Any], negative=False, bad=False):
        cross_attention_input = None
        cross_attention_masks = None
        add_cross_attention_input = None
        add_cross_attention_masks = None
        global_cond = None
        ...
        # Additional cross-attention conditioning (e.g., for trans_cap)
        if len(self.add_cross_attn_cond_ids) > 0:
            add_cross_attention_input = []
            add_cross_attention_masks = []

            for key in self.add_cross_attn_cond_ids:
                if key in conditioning_tensors:
                    add_cross_attn_in, add_cross_attn_mask = conditioning_tensors[key]

                    # Add sequence dimension if it's not there
                    if len(add_cross_attn_in.shape) == 2:
                        add_cross_attn_in = add_cross_attn_in.unsqueeze(1)
                        add_cross_attn_mask = add_cross_attn_mask.unsqueeze(1)

                    add_cross_attention_input.append(add_cross_attn_in)
                    add_cross_attention_masks.append(add_cross_attn_mask)

            if len(add_cross_attention_input) > 0:
                add_cross_attention_input = torch.cat(add_cross_attention_input, dim=1)
                add_cross_attention_masks = torch.cat(add_cross_attention_masks, dim=1)
            else:
                add_cross_attention_input = None
                add_cross_attention_masks = None
        ...
        if len(self.additional_cond_ids) > 0:
            # Sum all additional conditioning inputs over each element
            # Assumes that the additional conditioning inputs are of shape (batch, T, channels)
            for key in self.additional_cond_ids:
                additional_cond_input = conditioning_tensors[key][0]
                assert len(additional_cond_input.shape) == 3, f"Additional conditioning input {key} should be of shape (batch, T, channels), but got {additional_cond_input.shape}"
                if additional_cond is None:
                    additional_cond = additional_cond_input
                else:
                    additional_cond = additional_cond + additional_cond_input
        ...
        if not bad:
            if negative:
                return {
                    ...
                    "negative_add_cross_attn_cond": add_cross_attention_input,
                    "negative_add_cross_attn_mask": add_cross_attention_masks,
                    "negative_global_cond": global_cond,
                    "negative_additional_cond":additional_cond,
                ...
                return {
                    ...
                    "add_cross_attn_cond": add_cross_attention_input,
                    "add_cross_attn_mask": add_cross_attention_masks,
            ...
            if negative:
                return {
                    "bad_negative_cross_attn_cond": cross_attention_input,
                    "bad_negative_cross_attn_mask": cross_attention_masks,
                    "bad_negative_add_cross_attn_cond": add_cross_attention_input,
                    "bad_negative_add_cross_attn_mask": add_cross_attention_masks,
                    "bad_negative_global_cond": global_cond,
                    "bad_negative_input_concat_cond": input_concat_cond
                }
            else:
                return {
                    "bad_cross_attn_cond": cross_attention_input,
                    "bad_cross_attn_mask": cross_attention_masks,
                    "bad_add_cross_attn_cond": add_cross_attention_input,
                    "bad_add_cross_attn_mask": add_cross_attention_masks,
                    "bad_global_cond": global_cond,
                    "bad_input_concat_cond": input_concat_cond,
                    "bad_prepend_cond": prepend_cond,
                    "bad_prepend_cond_mask": prepend_cond_mask
                }

    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: tp.Dict[str, tp.Any], **kwargs):
        condition_embed = self.get_conditioning_inputs(cond)
        result = self.model(x, t, **condition_embed, **kwargs)
        return result
...
class DiTWrapper(ConditionedDiffusionModel):
    ...
    def forward(self,
        ...
        return self.model(
            ...
            add_cross_attn_cond_mask=add_cross_attn_mask,
            negative_add_cross_attn_cond=negative_add_cross_attn_cond,
            negative_add_cross_attn_mask=negative_add_cross_attn_mask,
            input_concat_cond=input_concat_cond,
            prepend_cond=prepend_cond,
            prepend_cond_mask=prepend_cond_mask,
            cfg_scale=cfg_scale,
            ...
            additional_embed=additional_cond,
            **kwargs)

class DiTWrapperCineDub(ConditionedDiffusionModel):
    def __init__(
        self,
        *args,
        **kwargs
    ):
        super().__init__(supports_cross_attention=True, supports_global_cond=True, supports_input_concat=False, supports_additional_cond=True)

        self.model = DiffusionTransformerCineDub(*args, **kwargs)
    ...
    def forward(self,
                ...
                bad_cross_attn_mask=None,
                bad_negative_cross_attn_cond=None,
                bad_negative_cross_attn_mask=None,
                bad_input_concat_cond=None,
                bad_negative_input_concat_cond=None,
                bad_global_cond=None,
                bad_negative_global_cond=None,
        ...
        return self.model(
            ...
            cross_attn_cond_mask=cross_attn_mask,
            negative_cross_attn_cond=negative_cross_attn_cond,
            negative_cross_attn_cond_mask=negative_cross_attn_mask,
            input_concat_cond=input_concat_cond,
            prepend_cond=prepend_cond,
            prepend_cond_mask=prepend_cond_mask,
            bad_cross_attn_cond=bad_cross_attn_cond,
            bad_cross_attn_cond_mask=bad_cross_attn_mask,
            bad_negative_cross_attn_cond=bad_negative_cross_attn_cond,
            bad_negative_cross_attn_cond_mask=bad_negative_cross_attn_mask,
            bad_input_concat_cond=bad_input_concat_cond,
            bad_prepend_cond=bad_prepend_cond,
            bad_prepend_cond_mask=bad_prepend_cond_mask,
            bad_global_embed=bad_global_cond,
            cfg_scale=cfg_scale,
            bad_cfg_scale=bad_cfg_scale,
            guidance_scale=guidance_scale,
            cfg_dropout_prob=cfg_dropout_prob,
            scale_phi=scale_phi,
            global_embed=global_cond,
            negative_global_embed=negative_global_cond,
            bad_negative_global_embed=bad_negative_global_cond,
            **kwargs)
...
def create_diffusion_cond_from_config(config: tp.Dict[str, tp.Any]):
    ...
    # Get add_cond_token_dim from config if add_cross_attention_cond_ids is specified
    add_cross_attention_cond_ids_config = diffusion_config.get('add_cross_attention_cond_ids', [])
    add_cond_token_dim = 0
    if len(add_cross_attention_cond_ids_config) > 0:
        # Use the same cond_dim from conditioning config
        conditioning_config = model_config.get('conditioning', None)
        if conditioning_config is not None:
            add_cond_token_dim = conditioning_config.get('cond_dim', 0)
    ...
    if diffusion_model_type == 'adp_cfg_1d':
        ...
        if "bad_model" in config:
            bad_model=deepcopy(config["bad_model"])
            diffusion_model=DiTWrapperCineDub(diffusion_objective=diffusion_objective,bad_model=bad_model,**diffusion_model_config)
        else:
            diffusion_model = DiTWrapper(diffusion_objective=diffusion_objective, add_cond_token_dim=add_cond_token_dim, **diffusion_model_config)
    
    # elif diffusion_model_type=="dit_wjy":
    #     bad_model=config["bad_model"]
    #     diffusion_model=DiTWrapperCineDub(diffusion_objective=diffusion_objective,bad_model=bad_model,**diffusion_model_config)
    ...
    add_cross_attention_ids = diffusion_config.get('add_cross_attention_cond_ids', [])
    global_cond_ids = diffusion_config.get('global_cond_ids', [])
    input_concat_ids = diffusion_config.get('input_concat_ids', [])
    prepend_cond_ids = diffusion_config.get('prepend_cond_ids', [])
    additional_cond_ids = diffusion_config.get('additional_cond_ids', [])
    ...
    return wrapper_fn(
        ...
        add_cross_attn_cond_ids=add_cross_attention_ids,
        global_cond_ids=global_cond_ids,
        input_concat_ids=input_concat_ids,
        prepend_cond_ids=prepend_cond_ids,
        additional_cond_ids=additional_cond_ids,
        pretransform=pretransform,
