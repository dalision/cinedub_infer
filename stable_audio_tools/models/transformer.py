...
def checkpoint(function, *args, **kwargs):
    import os
    # 通过环境变量控制是否启用梯度检查点
    # ENABLE_GRADIENT_CHECKPOINT=0 时直接调用 function，不使用 checkpoint（支持 torch.compile）
    # ENABLE_GRADIENT_CHECKPOINT=1 时启用 checkpoint（节省显存）
    if os.environ.get("ENABLE_GRADIENT_CHECKPOINT", "1") == "0":
...
class TransformerBlock(nn.Module):
    def __init__(
            ...
            cross_attend = False,
            dim_context = None,
            add_cross_attend = False,
            add_dim_context = None,
        ...
        # Additional cross attention (e.g., for semantic/trans_cap condition)
        self.add_cross_attend = add_cross_attend
        if add_cross_attend:
            self.add_cross_attend_norm = LayerNorm(dim, **norm_kwargs) if not remove_norms else nn.Identity()
            self.add_cross_attn = Attention(
                dim,
                dim_heads = dim_heads,
                dim_context=add_dim_context,
                causal = causal,
                zero_init_output=zero_init_branch_outputs,
                **attn_kwargs
            )
            self.add_cross_attn_scale = LayerScale(dim) if layer_scale else nn.Identity()
    ...
    def forward(
        ...
        if self.global_cond_dim is not None and self.global_cond_dim > 0 and global_cond is not None:
            tmp_global_cond = self.to_scale_shift_gate + global_cond if len(global_cond.shape) == 3  else (self.to_scale_shift_gate + global_cond).unsqueeze(1)
            scale_self, shift_self, gate_self, scale_ff, shift_ff, gate_ff = tmp_global_cond.chunk(6, dim=-1)
            ...
            # Additional cross attention (e.g., for semantic/trans_cap condition)
            if add_context is not None and self.add_cross_attend:
                x = x + self.add_cross_attn_scale(self.add_cross_attn(self.add_cross_attend_norm(x), context = add_context, context_mask = add_context_mask))
    ...
    def __init__(
        ...
        **kwargs
        ...
        for i in range(depth):
            ...
            should_add_cross_attend = add_cross_attend and (self.final_cross_attn_ix == -1 or i <= (self.final_cross_attn_ix))
            self.layers.append(
                TransformerBlock(
                    dim,
                    dim_heads = dim_heads,
                    ...
                    add_cross_attend = should_add_cross_attend,
                    add_dim_context = add_cond_token_dim,
