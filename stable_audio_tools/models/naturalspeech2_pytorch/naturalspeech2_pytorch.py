...
from stable_audio_tools.models.naturalspeech2_pytorch.attend import Attend
from stable_audio_tools.models.naturalspeech2_pytorch.aligner import Aligner, ForwardSumLoss, BinLoss
from stable_audio_tools.models.naturalspeech2_pytorch.utils.tokenizer import Tokenizer, ESpeak
from stable_audio_tools.models.naturalspeech2_pytorch.utils.utils import average_over_durations, create_mask
from stable_audio_tools.models.naturalspeech2_pytorch.version import __version__
...
try:
    # pyworld needs GLIBC_2.38 — optional on older hosts where the shared
    # library cannot be loaded. Inference (VTS audio prompt) does not need
    # pyworld pitch, so we tolerate the missing import silently.
...
class AudioToMel(nn.Module):
    ...
    def forward(self, audio):
        ...
        spectrogram = self.stft_transform(audio)
        mel = self.mel_transform(spectrogram)
...
class PhonemeEncoder(nn.Module):
    ...
    def forward(
        ...
        if is_bearable(x, List[str]):
            ...
            x = self.tokenizer.texts_to_tensor_ids(x).to(device)
...
class SpeechPromptEncoder(nn.Module):
    ...
    def __init__(
        self,
        dim_codebook,
        dims: Tuple[int] = (256, 2048, 2048, 2048, 2048, 768, 768, 768),
        *,
        depth = 6,
        heads = 8,
        dim_head = 64,
